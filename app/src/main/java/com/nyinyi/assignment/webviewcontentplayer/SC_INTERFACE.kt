...
private fun captureScreenshot(webView: WebView, callback: (String) -> Unit) {
    try {
        // Create bitmap of the entire WebView
        val bitmap = Bitmap.createBitmap(
            webView.width,
...
private fun sendScreenshotToJS(webView: WebView, base64String: String) {
    webView.post {
        webView.evaluateJavascript("""
            (function() {
                try {
                    if (typeof handleScreenshot === 'function') {
                        handleScreenshot('data:image/png;base64,$base64String');
                    } else {
                        console.error('handleScreenshot function not found');
                    }
                } catch(e) {
